...
4. Extend `printifyService` to list shops by account and sync a selected account. Update shop management UI to show account context, the assigned holder per shop, and avoid a global sync action. Hide the sync control for non-admins and hide readiness actions on shops the caller does not own.
...
8. Update `orderService.createDraft` to include `{ shop_id }` only when an admin selection exists. Ensure single and bulk calls use the same role-aware path.
9. Run the production build, inspect direct URL/permission behavior, and smoke-test account → user → order flows against the API.
...
| Seller order modal | Assigned shop title only; no picker/all-shop request | Manual smoke/build inspection |
...
- [ ] Seller/leader order modal shows assigned shop name and no selector.
...
- A UI-only hidden selector can be bypassed. Mitigation: API resolver remains authoritative and has spoofing tests.
- Deploying FE before API would send unsupported requests. Mitigation: API-first rollout and old-UI compatibility documented in plan.md.
